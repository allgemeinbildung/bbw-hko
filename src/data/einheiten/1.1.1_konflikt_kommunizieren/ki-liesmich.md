---
titel: "KI-Toolbox — Lies mich! (1.1.1 Konflikt kommunizieren)"
...
lebensbezug: "1.1"
quellen_json:
  - "1.1.1_konflikt_kommunizieren_ki.json"
  - "1.1.1_konflikt_kommunizieren_lernprompt.json"
  - "1.1.1_konflikt_kommunizieren_lernbegleiter.json"
---
...
## 1. Was in der Toolbox steckt — vier Dokumente, drei Funktionen

Alle vier liegen als **Word-Datei** im ZIP und sind frei anpassbar (siehe §3).

| # | Dokument | Funktion | Wann |
|---|---|---|---|
| **1** | KI-Auftrag «Die KI als Gegenseite im Konflikt» | **Üben mit KI** — die KI vertritt die Gegenposition, die Lernende prüft ihre eigene | mitten in der Einheit, nach Herausforderung A–C |
| **2** | KI-Auftrag «KI-Entwurf der Konfliktantwort redigieren» | **Üben mit KI** — KI schreibt einen Entwurf, die Lernende redigiert ihn kritisch | mitten in der Einheit, nach B/C |
...
> In dieser Einheit: **Rollen-Prompting**, **Kontext geben**, **Gegenposition fordern**, **Quellen und Belege anfordern**. Dazu zwei fertige Zwei-Schritt-Sequenzen («Stacking») und die Bauformel `[Rolle] + [Kontext] + [Aufgabe] + [Format]`.

---

## 2. Die eine Grundregel: KI prüft, sie ersetzt nicht

Alle vier Dokumente folgen demselben Prinzip — die KI **challengt, spiegelt und prüft** das eigene Produkt der Lernenden, sie liefert es nie fertig. Konkret heisst das: Jeder KI-Auftrag startet mit einem Schritt **«ohne KI»** (`ki_frei_vorher`), und der Lernbegleiter beginnt mit einer ehrlichen Selbsteinschätzung, bevor die KI ins Spiel kommt.

> [!warnung] Das ist die Leitplanke, nicht die Deko
> Auch in der kürzesten Variante (§3) bleibt der **«ohne KI zuerst»-Schritt drin**. Fällt er weg, üben die Lernenden, sich etwas vorschreiben zu lassen — genau das Gegenteil des Lernziels. Und: Im Kompetenznachweis sitzen sie allein da. Die KI darf hier üben helfen, nie die KN-Lösung schreiben.
...
Die Rückmeldung des Review-Teams war: die Dokumente sind inhaltlich dicht. Das ist gewollt (sie sollen ohne Nachschlagen tragen) — aber Sie müssen **nicht alles** einsetzen. Laden Sie die Word-Datei herunter und kürzen Sie gezielt:

> [!differenzieren] Rezept 1 — Eine Technik statt vier
> Im **KI-Lernprompt** behalten Sie **einen** Technik-Block (z. B. nur *Rollen-Prompting*) und löschen die anderen drei plus die beiden «Stacking»-Sequenzen. Die Lernenden üben eine Technik sauber, statt vier oberflächlich. Empfehlung als Einstieg: *Rollen-Prompting* oder *Quellen anfordern*.

> [!differenzieren] Rezept 2 — Nur ein KI-Auftrag
> Setzen Sie **entweder** Auftrag 1 (Gegenposition) **oder** Auftrag 2 (Redaktion) ein, nicht beide. Auftrag 1 schärft die Argumentation, Auftrag 2 das adressatengerechte Schreiben — wählen Sie nach Ihrem Schwerpunkt.
...
> Führen Sie **einen** Prompt live vor: Prompt eintippen, Antwort gemeinsam lesen, gemeinsam eine Rechtsaussage gegenprüfen. Die Lernenden sehen das kritische Prüfen einmal, bevor sie es selbst tun — das senkt die kognitive Last enorm und ist oft wirksamer als jedes Merkblatt.

> [!coaching] Gruppenpuzzle über die Techniken
> Vier Gruppen, je eine Prompt-Technik. Jede Gruppe probiert ihre Technik an der Einheits-Situation aus und erklärt sie den anderen. So wird die Dichte zur Stärke: niemand muss alle vier allein durcharbeiten.
...
## 5. Kurz-Checkliste vor dem Einsatz
...
Erzeugt aus der KI-Toolbox der Einheit 1.1.1 «Konflikt kommunizieren» (`ki.json`, `lernprompt.json`, `lernbegleiter.json`). Die KI-Schicht wird komplementär zur fertigen Einheit erzeugt (Skill `hko-ki-komplement`) und ändert die verbindlichen Unit-Dateien nie.
