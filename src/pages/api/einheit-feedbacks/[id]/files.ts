...
export const POST: APIRoute = async ({ locals, request, params }) => {
  ...
  return handleUpload(locals, request, id, 'einheit_feedbacks', locals.user.id)
}

export const DELETE: APIRoute = async ({ locals, request, params, url }) => {
  const id = params.id
  if (!id) return new Response(JSON.stringify({ error: 'ID fehlt.' }), { status: 400 })
  return handleDelete(locals, request, url, id, 'einheit_feedbacks')
}

export const GET: APIRoute = async ({ locals, params, url }) => {
  const id = params.id
  if (!id) return new Response(JSON.stringify({ error: 'ID fehlt.' }), { status: 400 })
  return handleGet(locals, url, id, 'einheit_feedbacks')
}
