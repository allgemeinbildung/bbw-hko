...
export const GET: APIRoute = async ({ params, request, locals }) => {
  ...
  if (!meta || !fullSet || !fullSet.kiLiesmich?.raw) {
    return new Response('KI-Toolbox-Liesmich nicht gefunden', { status: 404 })
  }
  ...
  const buffer = await buildBegleiterBuffer(fullSet.kiLiesmich.raw, logoPng)

  const filename = `${meta.kompetenz_nr}_${meta.slug}_ki-liesmich.docx`
