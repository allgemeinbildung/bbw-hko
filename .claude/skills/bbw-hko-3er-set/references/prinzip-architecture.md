...
- Spannt ein Spannungsfeld auf (Mehrdeutigkeit ist Pflicht)
...
> **Prosa-Begriff:** Die Spannungen heissen im sichtbaren Text **Zielkonflikt** (benennen/entscheiden) oder **Spannungsfeld** (die Spannung/der Raum) — nie „Trade-off". Der JSON-Key bleibt `trade_off` / `trade_off_raum`. Vollstaendige Regel: `language-rules.md` §5.4.
...
Jede der drei Herausforderungen aktiviert mindestens eines dieser Spannungsfelder in `mehrdeutigkeit.trade_off`. Die Hybrid-Herausforderung im KN aktiviert mindestens einen — typisch denselben, der die thematisch dichteste Spannung traegt.
