...
**Diese Skill arbeitet ausschliesslich mit `public/nrlp_2j.json`.** Die EBA-Themen
sind **eigenstaendig** — sie tragen andere Titel und eine andere Reihenfolge als
die EFZ-Themen. Eine Themen-Nummer aus dem EFZ-Lehrplan bedeutet hier etwas
anderes; nie zwischen den Lehrgaengen uebertragen.

Alle acht Themen sind vollstaendig ausgearbeitet (finaler Erlass Bildungsrat
2026-06). Kompetenzen, Aspekte und Sprachmodi werden **nachgeschlagen, nie
hergeleitet.** Alle Themen haben je 22 Lektionen und 2 Lebensbezuege.

| Nr | Titel | LJ |
|----|-------|----|
| 1 | Ins Berufsleben einsteigen | LJ1 |
| 2 | Bewusst konsumieren und handeln | LJ1 |
| 3 | Sicherheit und Gesundheit | LJ1 |
| 4 | Medien und digitale Welt | LJ1 |
| 5 | Meinung bilden und mitgestalten | LJ2 |
| 6 | Vertraege verstehen – fair handeln | LJ2 |
| 7 | Arbeit und Zukunft | LJ2 |
| 8 | Kultur und Kunst | LJ2 |

EBA kennt **kein** Schlussarbeits-Thema; alle acht Themen haben Lebensbezuege und
sind in der Zirkularitaets-Spirale eingetragen.

> **Diese Tabelle enthielt bis 2026-08-16 die EFZ-Themen** (aus der 3er-Skill
> uebernommen) plus die Behauptung, T4–T7 seien im „Skizze-Status" und Kompetenzen
> muessten „aus Spiralcurriculum-Logik abgeleitet" werden. Beides war falsch und
> haette dazu gefuehrt, EBA-Sets gegen den falschen Lehrplan zu bauen. Immer
> `nrlp_2j.json` lesen.
...
| K3 | Positionieren, Entscheiden | Entscheidungsproblem | „Wie positionierst du dich zu privaten Social-Media-Posts am Lehrplatz?" |
...
#### EBA-2er-Override (`lehrgang=EBA_2J`)

Beim 2er-Set hat `persona_pool_units` nur **2 Berufe** (statt 3):

- **persona_pool_units (2 Berufe):** MUESSEN aus **mindestens 2 verschiedenen Abteilungen**
  stammen (maximaler Kontrast — Spiralen-Regel). Doppelung derselben Abteilung in den 2
  Units-Personas ist verboten → `ERR_PERSONA_ABTEILUNG_MONO` (Schwelle 2 statt 3).
- **persona_pool_kn_neu (2 Berufe):** unveraendert (2+2, disjunkt, >=1 in Units abwesende Abteilung).
- **EBA-Berufe bevorzugen** — Schreibweise **string-identisch** aus der kanonischen EBA-Tabelle unten
  (nicht aus der EFZ-Spalte). In der Prosa als EBA-Beruf benennen.
- **Lehrjahr:** Personas im 1.–2. Lehrjahr (EBA hat nur LJ1-2).
- **Abteilungs-Mix (EBA-2er):** `persona_pool_units` (2 Berufe) aus **>=2** verschiedenen Abteilungen;
  `persona_pool_kn_neu` (2 Berufe) aus mindestens einer in den Units **nicht** vertretenen Abteilung.
  Die EFZ-Regel (>=3 Abteilungen) bleibt unveraendert daneben gueltig.

#### Kanonische EBA-Lehrberufe (mit Umlauten) — verbindlich fuer `lehrgang=EBA_2J`

`persona.beruf` muss bei EBA **string-identisch** mit der Spalte „EBA-Schreibweise (mit Umlauten)"
sein (kein Fuzzy-Matching, keine Transliteration in der Prosa). IDs/Slugs werden wie bei EFZ
transliteriert (ae/oe/ue, ss). Quelle aller Eintraege: berufsberatung.ch (Steckbriefe / Berufs-Check)
bzw. SBFI-Berufsverzeichnis, geprueft 2026-06-17.

| ID | Abteilung | EBA-Schreibweise (mit Umlauten) | Quelle |
|---|---|---|---|
| maler | Bau | Malerpraktiker/in EBA | berufsberatung.ch |
| maurer | Bau | Baupraktiker/in EBA | berufsberatung.ch (id 7946) |
| plattenleger | Bau | Plattenlegerpraktiker/in EBA | berufsberatung.ch (id 7947) |
| schreiner | Bau | Schreinerpraktiker/in EBA | berufsberatung.ch (id 6823) |
| spengler | Bau | Spenglerpraktiker/in EBA | berufsberatung.ch |
| holzbearbeiter | Bau | Holzbearbeiter/in EBA | berufsberatung.ch (id 7964) |
| automobilfachmann | Technik / Ernaehrung | Automobilassistent/in EBA | berufsberatung.ch |
| baecker_konditor | Technik / Ernaehrung | Bäcker-Konditor-Confiseur/in EBA | berufsberatung.ch |
| polymechaniker | Maschinenbau | Mechanikpraktiker/in EBA | berufsberatung.ch |

**Informatik-Abteilung:** Es ist **kein** Standard-EBA-Beruf gesichert (Informatik ist EFZ-zentriert;
„Informatikpraktiker/in EBA" liess sich **nicht** verifizieren). Der Abteilungs-Mix-Check darf fuer
EBA **nicht** auf die Informatik-Abteilung verweisen, solange hier kein verifizierter Beruf steht.

**TODO (vor breitem Rollout, von Pietro/Claude-Code zu verifizieren und zu ergaenzen):**
- Technik/Ernaehrung: weitere EBA (z. B. Kuechenangestellte/r EBA, Lebensmittelpraktiker/in EBA) —
  offizielle Bezeichnung pruefen, dann eintragen.
- Bau: Strassenbaupraktiker/in EBA u. a. — pruefen.
- Maschinenbau: weitere EBA neben Mechanikpraktiker — pruefen.
- Informatik: klaeren, ob ueberhaupt ein EBA existiert; falls nein, dauerhaft vermerken.
- Regel: nur mit offiziell verifizierter Bezeichnung (berufsberatung.ch / SBFI) eintragen — sonst hier belassen.
...
Die 5 Herausforderungen einer 5er-Gruppe (Phase 0.5) erben diese 8 Merkmale — jede Herausforderung muss alle 8 abdecken koennen. Im 2er-EBA-Set sind es **2 Herausforderungen (A/B)** — beide muessen alle 8 Merkmale abdecken.

---

## 13. EBA-2er-Spezifika (`lehrgang=EBA_2J`)

Diese Skill ist der EBA-Fork. Wo dieses Framework EFZ-Werte nennt, gilt fuer EBA Folgendes
(Quellen: `rahmenlehrplan-allgemeinbildung-9-april-25.txt`, `nrlp_2j.json`, Konzept-Handoff):

**Quoten (RLP Z.791, Pro-Thema-Minimum):** **2 SK · 2 Aspekte · 1 Sprachmodus.** Schwerpunkt-Modi:
Rezeption muendlich/schriftlich, Produktion + Interaktion **muendlich** → darum ist das Fachgespraech
die KN-Primaerform. Kumulativ bis Abschluss: alle SK + alle 8 Aspekte ueber die 8 Themen.

**Konventionen-Set (§9):** EBA = **Konventionen + Sprachbewusstheit, KEINE Normen.** Diese werden
gemaess §6 **in die SuK-Kriterien integriert** — die Kriterien-Namen bleiben „Fachkorrektheit" +
„Argumentation" (kein eigenes Konventionen-/Sprachbewusstheit-Kriterium).

**Keine Schlussarbeit, keine Schlusspruefung** (RLP §6.2/6.3 betreffen nur 3J/4J). Es gibt kein
SA-Sammelbecken — **der KN pro Einheit traegt alles.** Die EBA-SA-Zeile (2/2/3) in Quick Ref §3
betrifft nur QV-Wiederholer/Externe (RLP Z.716), nicht regulaere EBA-Lernende — **keine**
Designgrundlage hier.

**8 EBA-Themen** (aus `nrlp_2j.json`, je 22 Lektionen, je 2 Lebensbezuege; flach, gleichgewichtet):
...
**K-Fenster:** K2-K3, K3 als Decke, K4 nur als optionale 100%-Extension (nicht im Pflicht-Versprechen).

**Persona-Mix:** `persona_pool_units` 2 Berufe aus >=2 Abteilungen (siehe §11 EBA-Override), LJ1-2,
EBA-Berufe bevorzugt.

**Sprachniveau der Lernenden:** Dossier + Auftraege **max. A2 GER**, hart erzwungen (siehe
`a2-language-rules.md`).

**Kein Lehrmittel:** Das Wissen liefert das separat generierte **Dossier** (`dossier-template.json`,
`dossier-architecture.md`), Backward Design in Phase 7.
