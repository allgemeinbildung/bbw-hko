...
|---|---|---|
| Prosa-Text (situation_text, leitfragen, etc.) | **JA, Pflicht nativ** | „Du überlegst, ob du den Schuh kaufst." (NICHT „ueberlegst") |
...
**Artikel/Pronomen bleiben unverändert** — gegendert wird nur das Substantiv. `dein Berufsbildner/in`, `des Berufsbildner/in`, `als Lernende/r`. Das Gendern von Artikeln/Pronomen (`der/die`, `sie/er`) ist bewusst nicht Teil dieser Regel (sonst Satzumbau statt Wort-Ersetzung).
...
- Beispiel: „Benennt die Lernende beide Lesarten des Trade-offs sprachlich präzise?"
...
"der Trade-off" → in KN-Kontext OK (Fachbegriff), aber sparsam
...
Trade-off bleibt — er ist Skill-1-Fachbegriff für Mehrdeutigkeits-Anker. Andere Anglizismen vermeiden.
...
**Heuristik:** Ein guter Trade-off-Hint benennt beide Pole **konkret und kurz**, ohne der Lernenden die Wahl abzunehmen.
...
„Rechen-Fehler im Detail dulden, wenn Trade-off-Konzept erkannt wurde."
                                                        (70 Zeichen)
...
```
„Es ist wichtig, dass die Lernende beide Pole des Trade-offs sieht und sie 
...
□ Trade-off-Hint benennt beide Pole konkret
