...
- Ein Trade-off-Aktivator — mind. ein Eintrag aus `prinzip.mehrdeutigkeits_architektur.trade_off_raum` spannt sich auf
...
| 3 | Beurteilen | K3 | "Wuerdest du {beide Aspekte} gleich bewerten — warum oder warum nicht?" |
| 4 | Transfer | K4 | "Vergleiche mit einer der drei Lernaufgaben — wo ist das Prinzip gleich, wo unterschiedlich?" |
...
1. "Welches Grundprinzip hast du in deiner Herausforderung gelernt — formuliere es in einem Satz."
2. "Erklaere, wie dieses Prinzip in der Hybrid-Herausforderung sichtbar wird. Was ist gleich, was ist anders?"
3. "Wann versagt das Prinzip — und was hast du durch die drei Herausforderungen insgesamt darueber gelernt?"
...
Diese Datei ist die EBA-Fassung. KN-Mechanik identisch; nur die folgenden Werte aendern sich.
Ueberall, wo oben „drei/A/B/C", „K4" oder „Lehrmittel" steht, gilt fuer EBA:

**Hybrid-Herausforderung (§1/§2):** kombiniert **beide** Herausforderungen (A+B),
`must_combine [A,B]`, `herausforderungen_mapping` hat **2 Eintraege** (A/B). Persona aus
`persona_pool_kn_neu`, LJ1-2. max. 120 Woerter, ICH, **A2** (Satz <= 18 W., Fachbegriffe nur mit
Dossier-Glossar).

**KN-Primaerform: Fachgespraech.** Muendlich ist EBA-Schwerpunkt (RLP Z.791: Produktion +
Interaktion muendlich). Mini Case + Werkschau sind **vereinfachte Alternativen**, nicht gleichrangig
beworben — die LP waehlt, Default ist Fachgespraech.

**K-Decke K3 (alle Typen):**

- **Fachgespraech (§3):** 25-30 Min. total, 10-15 Min. Vorbereitung. Fragestruktur-K-Stufen
  K2/K2/K3/K3/K3 (statt …/K4/K4). Frage 4 „Transfer" und Frage 5 „Werthaltung" bleiben, aber auf K3.
  Ablauf: **„Dossier erlaubt, kein Internet"** statt Lehrmittel. SK = Union ueber **A/B**.
- **Mini Case (§4):** **30-40 Min.** (statt 45-60), 4 kurze Aufgaben mit K-Stufen K2/K2/K3/K3
  (Aufgabe 4 von K4 auf K3). **Mehr Scaffold direkt im Pruefungsblatt** (Satzanfaenge/Struktur aus
  dem Dossier eingedruckt). Ablauf: „Dossier erlaubt; kein Internet".
- **Werkschau (§5):** Lernende waehlen eines ihrer **beiden** Handlungsprodukte (A/B).
  Transfer-Reflexion **120-150 Woerter** (statt 200-250), **stark gefuehrt** mit Satzanfaengen aus
  dem Dossier. Reflexionsfragen referenzieren „beide Herausforderungen".

**Bi-dimensionale Rubrik (§6) — SuK-Kriterien INTEGRIERT (Entscheid Pietro/KT1):** Die zwei
SuK-Kriterien behalten die Namen **„Fachkorrektheit"** + **„Argumentation"** wie im EFZ. Konventionen
+ Sprachbewusstheit werden NICHT als eigene Kriterien gefuehrt, sondern **in diese beiden integriert**
(z.B. „Fachkorrektheit inkl. sprachlicher Konventionen", „Argumentation inkl. Sprachbewusstheit") —
gemaess §6 dieses Dokuments + hko-framework §6/§9. EBA-Unterschied: **KEINE Normen** (nur EFZ). Die zwei
Ges-Kriterien bleiben: Kriterium 3 = aspektabhaengiges „…-Prinzip" (Default-Wording „Gesellschafts-
Prinzip"; Aspekt-Mapping aus §6 unveraendert), Kriterium 4 = „Position / Werthaltung". 4 Stufen,
gleichgewichtet, Niveaubaender unter 60 / 80 / 100 unveraendert.

**SK11 (§7):** Mehrdeutigkeit Pflicht in **2 von 2** Herausforderungen, aber **gefuehrt** — das
Dossier-Transfer-Wissensblatt macht den Trade-off sichtbar.

**Wissen↔KN-Alignment (NEU):** Jeder KN-Anspruch (jede Frage/Aufgabe/Reflexionsfrage + das
Kernprinzip) muss im Dossier gedeckt sein, sonst `ERR_DOSSIER_GAP` (siehe
`dossier-architecture.md` §6).
