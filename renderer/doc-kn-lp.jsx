/* doc-kn-lp.jsx — KN Durchfuehrung Lehrperson + Bewertung */
/* globals React, A4Page, sitColors, Badge, SectionHead */

function LpPage({ kn, abteilung, docCode, pageNum, pageTotal, docTitel, children }) {
  return (
    <A4Page
      sit={null}
      abteilung={abteilung}
...
function DocKnLpKontext({ kn, prinzip, set, abteilung, pageNum, pageTotal }) {
  return (
...
function DocKnLpHybrid({ kn, abteilung, pageNum, pageTotal }) {
...
function DocKnLpDurchfuehrungIntro({ kn, abteilung, pageNum, pageTotal }) {
  return (
...
function DocKnLpKnTyp({ kn, abteilung, knTyp, sectionNr, pageNum, pageTotal }) {
  return (
    <LpPage kn={kn} abteilung={abteilung} docCode="DOC-KN-LP · DURCHFUEHRUNG"
            docTitel={kn.hybrid_situation?.titel}
            pageNum={pageNum} pageTotal={pageTotal}>
...
function DocKnLpBewertung({ kn, abteilung, pageNum, pageTotal }) {
...
function DocKnLp({ kn, prinzip, set, abteilung }) {
  const style = sitColors(null);
  const kt = kn.kn_typen || [];

  // Pages: 1 Kontext, 2 Hybrid, 3 Durchf-Intro+Typ1, 4 Typ2, 5 Typ3, 6 Bewertung
