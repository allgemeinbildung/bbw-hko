...
// Draw the highlight ring at a known viewport bounding box.
export async function highlightAtBox(page, box, label) {
  if (!box) return;
  await page.evaluate(
    ({ box, label, containerId }) => {
      const r = { left: box.x, top: box.y, width: box.width, height: box.height };
    ...
    { box, label: label ?? null, containerId: CONTAINER_ID },
  );
}

export async function highlight(page, selector, label) {
  // Resolve the element via Playwright's locator API (handles >> nth=N etc.)
  // then delegate to highlightAtBox with the viewport-relative bounding rect.
  let box = null;
  try {
    box = await page.locator(selector).first().boundingBox({ timeout: 5000 });
  } catch { return; }
  await highlightAtBox(page, box, label);
}

// Legacy path kept for back-compat — used by old callers that pass raw CSS.
async function _legacyHighlight(page, selector, label) {
  await page.evaluate(
